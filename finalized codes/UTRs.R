...
#stores into UTRlist, loci previously efined in framework 
UTRlist<-list()

for (i in 1:nrow(atlas)){
UTRlist[[atlas$locus[i]]]<-featureselect(i ,2, splitDFs[[i]])}
...
UTRisolated<-list()
for(i in 1:nrow(atlas)){
UTRisolated[[atlas$locus[i]]]<-GFEpaster(UTRlist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(atlas$locus[i], "*00:00"), " ", ""))}
