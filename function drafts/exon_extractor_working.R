...
for(i in 1:length(loci)){
  ...
  colnames(AA_segments[[i]]) <- c("locus","allele","trimmed_allele","allele_name",seq(-24,ncol(AA_segments[[i]][,5:ncol(AA_segments[[i]])])+1-1))}
...
value<-exonlist[[1]][match(colnames(exonlist[[1]][[1]]), corr_table[[1]][1,])]
value[sapply(value, is.null)] <- NULL
