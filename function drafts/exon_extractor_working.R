#exon_extractor working script 
#V 0.7 - 10/29/18 -- unpolished
#By: Livia Tran
...
loci="B"
  ...
  actual_subset<-AA_segments<-AA_aligned <-refexon<-pepsplit<-alignment<-exonlist<- sapply(loci, function(x) NULL)
  ...
  for(i in 1:length(AA_segments)){
    for(j in 1:nrow(corr_table[[match(loci[i],names(corr_table))]])){
      actual_subset[[i]][[j]]<-cbind(AA_segments[[i]][,1:4], AA_segments[[i]][match(corr_table[[match(loci[i],names(corr_table))]][j,6],colnames(AA_segments[[loci[i]]])):match(corr_table[[match(loci[i],names(corr_table))]][j,7],colnames(AA_segments[[loci[i]]]))])}}

  
if(corr_table[[match(loci[i],names(corr_table))]][2,6]%in%colnames(AA_segments[[i]])==FALSE)
...
AA_atlas[[match(loci[i],names(AA_atlas))]][AA_atlas[[match(loci[i],names(AA_atlas))]]==""]<-as.numeric(length(AA_segments[[i]]))-4
...
#binds previous columns with locus, allele, trimmed allele, and allele name information 
    for(i in 1:length(AA_segments)){
      for(j in 1:nrow(AA_atlas[[match(loci[i],names(AA_atlas))]])){
        exonlist[[i]][[j]]<-cbind(AA_segments[[i]][,1:4], AA_segments[[i]][match(AA_atlas[[match(loci[i],names(AA_atlas))]][j,2],colnames(AA_segments[[loci[i]]])):match(AA_atlas[[match(loci[i],names(AA_atlas))]][j,3],colnames(AA_segments[[loci[i]]]))])}}


View(AA_atlas[[2]])
##END FUNCTION SCRIPT 
x$pepseq[match(y[,1], x[,1])]
