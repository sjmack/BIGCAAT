...
for(b in 1:length(nonCWDtrunc)){
  genotype_variants[[d]][[b]]<-na.omit(subset(geno_exonlist[[d]], geno_exonlist[[d]]$trimmed_allele==nonCWDtrunc[[b]]))
...
#binds narrowed down non-CWD alleles for one amino acid exons and inputs it back
#need to find a way to avoid hardcoding to exon 8
all_gdata[[8]]<-rbind(all_gdata[[d]][ncol(all_gdata[[d]])==7], rbind(nonCWD_checked[[1]], nonCWD_checked[[2]]))
    }}
  ...
  #removes invariatn positions (i.e elements with no rows )
  position_parsed<-position_parsed[sapply(position_parsed, nrow)>0]
  
  #creates a dataframe that will go into BIGDAT, 
  ...
  mastertable<- data.frame(gdata[,c(1,2)], matrix("", ncol = length(position_parsed)*2))
  mastertablecols<-paste(loci[i], names(position_parsed), sep="_")

  #creates a repeat of each amino acid variant for mastertable column names 
  rep_variantAA<-NULL
  
  for(t in 1:length(mastertablecols)){
    rep_variantAA[[t]]<-rep(mastertablecols[[t]],2)}
  
  #renames column names 
  colnames(mastertable)<-c("SampleID", "Disease", unlist(rep_variantAA))
  
  #extracts which alleles in the genotype data are not in the amino acid specific variants 
  missing_geno<-sapply(position_parsed, function(x) NULL)

  for(f in 1:length(position_parsed)){
  missing_geno[[f]]<-geno_alleles[!geno_alleles %in% position_parsed[[f]]$trimmed_allele]}


#creates a new dataframe for each of the missing alleles with NA input
missing_geno_output<-NULL
for(h in 1:length(missing_geno)){
  missing_geno_output[[h]]<-data.frame("V1"=missing_geno[[h]],
                "V2"=NA, stringsAsFactors = F)
  
  #renames dataframes so the missing allele data can be bound respectively for each
  #amino acid variant
colnames(missing_geno_output[[h]])<-c("trimmed_allele", names(position_parsed[[h]][2]))

position_parsed[[h]]<-rbind(position_parsed[[h]],missing_geno_output[[h]])

#matches each locus specific column in gdata to each variant amino acid position -- inputs
#into appropriate column in master table 
  for(j in 1:length(gdata[loci==colnames(gdata)])){
mastertable[names(mastertable)==paste(loci[i], names(position_parsed[[h]][2]), sep="_")][j]<-position_parsed[[h]][,2][match(gdata[loci[i]==colnames(gdata)][[j]], position_parsed[[h]][,1])]}
}}
