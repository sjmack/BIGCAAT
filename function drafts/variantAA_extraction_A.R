...
for(i in 1:length(loci)){
  ...
  if(loci[i]=="B"){
    w$B<-capture.output(cat(alignment_start[[i]]:(as.numeric(refblock_number[[i]][4])-(alignment_start[[i]])), (as.numeric(refblock_number[[i]][4])-alignment_start[[i]]+1):alignment_length[[i]]))}
  if(loci[[i]]=="C"){
...
for(g in 1:length(position_parsed)){
    position_parsed[[g]]<-subset(position_parsed[[g]], length(unique(position_parsed[[g]][,2]))!=1)}
...
#into appropriate column in master table 
  for(u in 1:length(gdata[loci==colnames(gdata)])){
mastertable[names(mastertable)==paste(loci[i], names(position_parsed[[h]][2]), sep="_")][u]<-position_parsed[[h]][,2][match(gdata[loci[i]==colnames(gdata)][[u]], position_parsed[[h]][,1])]}
}}



BIGDAWG(mastertable, HLA=F, Run.Tests="L", Missing = "ignore")
