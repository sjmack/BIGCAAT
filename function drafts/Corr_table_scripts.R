...
for(i in 1:length(loci)) {
  ...
  for (j in 1:length(prot_start)){
    ...
    if(loci[[i]]=="A"){
    w<-capture.output(cat(alignment_start[[i]]:(as.numeric(refblock_number[[j]][4])-(alignment_start[[i]])), (as.numeric(refblock_number[[i]][4])-alignment_start[[i]]+1):alignment_length[[i]]))}
    if(loci[[i]]=="C"){
