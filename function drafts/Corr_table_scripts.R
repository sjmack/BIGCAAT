###Correspondence table for HLA-A and HLA-C
#By: Livia Tran
#11/3/2018
#v 0.3
...
cols<-w<-end<-corr_table<-downloaded_segments<-alignment_positions<-alignment_length<-alignment_start<-downloaded<-prot_extractions<-diff<-prot_start<-end_char<-refblock_number<-sapply(loci, function(x) NULL)
...
for(i in 1:length(loci)) {
  ...
  downloaded[[loci[i]]] <- readLines(paste("https://raw.githubusercontent.com/ANHIG/IMGTHLA/Latest/alignments/",paste(loci[[i]],"_prot.txt",sep=""),sep=""),-1,ok=TRUE,skipNul = FALSE)
  ...
  for (j in 1:length(prot_start[[i]])){
    prot_extractions[[i]][j]<-strsplit(downloaded[[i]][prot_start[[i]][j]], " ")
  ...
  #determines the alignment start by adding -30 to the difference between white spaces found above 
    alignment_start[[loci[i]]]<-refblock_number[[i]][1]+diff[[i]]
...
for(i in 1:length(loci)){
  ...
  downloaded_segments[[i]]<-cbind(downloaded_segments[[i]], downloaded[[i]][,2][prot_start[[i]][k]:end[[i]][k]])}

##stopped here
  cols[[i]]<-seq(1, ncol(downloaded_segments[[i]]), by=1)
  

downloaded_segments[[i]]<-apply(downloaded_segments[[i]][,cols[[i]]] , 1 , paste , collapse = "" )
...
#creates a new matrix with the number of columns equal to the number of characters in the reference sequence 
  corr_table[[i]]<-matrix(, nrow = 2, ncol = as.numeric(nchar(downloaded_segments[[i]][3])))

  #contains actual sequence information
  corr_table[[i]][1,]<-(1:as.numeric(nchar(downloaded_segments[[i]][3])))
