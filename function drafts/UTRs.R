...
#defines loci in preparation for UTRlist 
...
#stores into UTRlist, loci defined above 
for (i in 1:nrow(atlas)){
UTRlist[[loci[i]]]<-featureselect(i ,2, splitDFs[[i]])}
...
UTRisolated<-list()
for(i in 1:nrow(atlas)){
UTRisolated[[loci[i]]]<-GFEpaster(UTRlist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(loci[1], "*00:00"), " ", ""))}
