####Feature Sequence Isolation for Leader Peptide GFEs 
##By: Livia Tran 
...
#stores into LeaderPeplist, loci previously efined in framework 
LeaderPeplist<-list()

for (i in 1:nrow(atlas)){
  LeaderPeplist[[atlas$locus[i]]]<-featureselect(i ,5, splitDFs[[i]])}
...
LeaderPepisolated<-list()
for(i in 1:nrow(atlas)){
  LeaderPepisolated[[atlas$locus[i]]]<-GFEpaster(LeaderPeplist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(atlas$locus[i], "*00:00"), " ", ""))}
