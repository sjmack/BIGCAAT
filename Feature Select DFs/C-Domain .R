####Feature Sequence Isolation for C-Domain GFEs 
##By: Livia Tran 
...
#stores into Cdomainlist, loci previously efined in framework 
Cdomainlist<-list()

for (i in 1:nrow(atlas)){
  Cdomainlist[[atlas$locus[i]]]<-featureselect(i ,6, splitDFs[[i]])}
...
Cdomainisolated<-list()
for(i in 1:nrow(atlas)){
  Cdomainisolated[[atlas$locus[i]]]<-GFEpaster(Cdomainlist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(atlas$locus[i], "*00:00"), " ", ""))}
