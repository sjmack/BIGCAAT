####Feature Sequence Isolation for Transmembrane Exon GFEs 
##By: Livia Tran 
...
#stores into TMexonlist, loci previously efined in framework 
TMexonlist<-list()

for (i in 1:nrow(atlas)){
  TMexonlist[[atlas$locus[i]]]<-featureselect(i ,8, splitDFs[[i]])}
...
TMisolated<-list()
for(i in 1:nrow(atlas)){
  TMisolated[[atlas$locus[i]]]<-GFEpaster(TMexonlist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(atlas$locus[i], "*00:00"), " ", ""))}
