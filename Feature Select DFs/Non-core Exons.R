####Feature Sequence Isolation for Non-core Exon GFEs 
##By: Livia Tran 
...
#stores into NCexonslist, loci previously efined in framework 
NCexonslist<-list()

for (i in 1:nrow(atlas)){
  NCexonslist[[atlas$locus[i]]]<-featureselect(i ,9, splitDFs[[i]])}
...
NCisolated<-list()
for(i in 1:nrow(atlas)){
  NCisolated[[atlas$locus[i]]]<-GFEpaster(NCexonslist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(atlas$locus[i], "*00:00"), " ", ""))}
