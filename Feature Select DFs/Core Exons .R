...
#stores into CoreExonlist, loci previously efined in framework 
CoreExonlist<-list()

for (i in 1:nrow(atlas)){
  CoreExonlist[[atlas$locus[i]]]<-featureselect(i ,3, splitDFs[[i]])}
...
CoreExonisolated<-list()
for(i in 1:nrow(atlas)){
  CoreExonisolated[[atlas$locus[i]]]<-GFEpaster(CoreExonlist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(atlas$locus[i], "*00:00"), " ", ""))}
