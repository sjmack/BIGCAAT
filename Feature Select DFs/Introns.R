...
#stores into Intronlist, loci previously efined in framework 
Intronlist<-list()

for (i in 1:nrow(atlas)){
  Intronlist[[atlas$locus[i]]]<-featureselect(i ,7, splitDFs[[i]])}
...
Intronisolated<-list()
for(i in 1:nrow(atlas)){
  Intronisolated[[atlas$locus[i]]]<-GFEpaster(Intronlist, i, splitDFs[[i]], hladf, i, str_replace_all(paste(atlas$locus[i], "*00:00"), " ", ""))}
